...
export class AdminEmployeesAddComponent {
    ...
    passwordRepeatValidator(form: FormGroup): {[s: string]: boolean} {
        if (form.value['password'] != form.value['passwordRepeat']) {
            form.controls['passwordRepeat'].setErrors({notEquivalent: true});
        }
        return null;
    }
    ...
    submit() {
        this.loading = true;
        this.employeeService.push(new Employee(this.employeeFormGroup.value))
